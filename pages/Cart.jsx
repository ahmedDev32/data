import { GetSubtotal } from '@/Models/utilityfunction'
...
const Cart = () => {
  ...
  fetchCartData()
    const handleStorageChange = (event) => {
      if (event.key === "Cart") {
        let value = GetSubtotal()
        SetSubTotal(value)
        const CartDatas = JSON.parse(event.newValue);
        if (CartDatas !== null) {
          if (!Array.isArray(CartData)) {
            SetCartData([CartDatas]);
          } else {
  ...
  let SetIncreament = (id) =>{
    let UpdateQty = CartData.map((item)=>{
      if(item.id === id ){
  ...
  return (
    <>
    <section className="bg-white my-2 py-4 antialiased  md:py-16">
  <div className="mx-auto max-w-screen-xl px-4 2xl:px-0">
    <h2 className="text-xl font-semibold text-gray-900  sm:text-2xl">&quot;Shopping Cart Your Cart Have {CartData && CartData.length || 0 } Items&quot;</h2>
...
<div key={index} className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800 md:p-6">
                ...
                <div className="flex items-center">
                  <button onClick={()=>{
                    ...
                    <svg className="h-2.5 w-2.5 text-gray-900 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 18 2">
                      <path stroke="currentColor" stokeLinecap="round" stokeLinejoin="round" stokeWidth="2" d="M1 1h16" />
                    </svg>
                  </button>
                  <input type="text" id="counter-input" data-input-counter className="w-10 shrink-0 border-0 bg-transparent text-center text-sm font-medium text-gray-900 focus:outline-none focus:ring-0 dark:text-white" placeholder="" value={item.qty} required />
                  <button onClick={()=>{
                    SetIncreament(item.id)
                  }} type="button" id="increment-button" data-input-counter-increment="counter-input" className="inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-md border border-gray-300 bg-gray-100 hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-100 dark:border-gray-600 dark:bg-gray-700 dark:hover:bg-gray-600 dark:focus:ring-gray-700">
                    <svg className="h-2.5 w-2.5 text-gray-900 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 18 18">
                      <path stroke="currentColor" stokeLinecap="round" stokeLinejoin="round" stokeWidth="2" d="M9 1v16M1 9h16" />
                    </svg>
                  </button>
                </div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <button onClick={()=>{
                    ...
                    <svg className="me-1.5 h-5 w-5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                      <path stroke="currentColor" stokeLinecap="round" stokeLinejoin="round" stokeWidth="2" d="M6 18 17.94 6M18 18 6.06 6" />
                    </svg>
                    Remove
                  </button>
                </div>
      ...
      <div className="mx-auto mt-6 max-w-4xl flex-1 space-y-6 lg:mt-0 lg:w-full">
        <div className="space-y-4 rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700  sm:p-6">
          ...
          <Link href="/CheckOut" className="flex w-full items-center justify-center rounded-lg bg-gray-700 px-5 py-2.5 text-sm font-medium text-white hover:bg-gray-800 focus:outline-none focus:ring-4 focus:ring-gray-300 dark:bg-gray-600 dark:hover:bg-gray-700 dark:focus:ring-gray-800">Proceed to Checkout</Link>

          <div className="flex items-center justify-center gap-2">
            <span className="text-sm font-normal text-gray-500 dark:text-gray-400"> or </span>
            <Link href="/Products?p=Surgical" title="" className="inline-flex items-center gap-2 text-sm font-medium text-gray-700 underline hover:no-underline dark:text-gray-500">
              Continue Shopping
              <svg className="h-5 w-5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                <path stroke="currentColor" stokeLinecap="round" stokeLinejoin="round" stokeWidth="2" d="M19 12H5m14 0-4 4m4-4-4-4" />
              </svg>
            </Link>
          </div>
        </div>
        ...
        {/* <div className="space-y-4 rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800 sm:p-6">
          <form className="space-y-4">
            ...
            <button type="submit" className="flex w-full items-center justify-center rounded-lg bg-gray-700 px-5 py-2.5 text-sm font-medium text-white hover:bg-gray-800 focus:outline-none focus:ring-4 focus:ring-gray-300 dark:bg-gray-600 dark:hover:bg-gray-700 dark:focus:ring-primary-800">Apply Code</button>
          </form>
        </div> */}
      </div>
    </div>
